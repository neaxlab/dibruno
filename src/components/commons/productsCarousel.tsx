...
export default function ProductsCarousel({ products }: { products: any }) {
    ...
    useEffect(() => {
        if (swiperRef.current && scrollbarRef.current) {
            ...
            const updateScrollbar = () => {
                ...
                const slidesPerView = typeof slidesPerViewParam === 'number' ? slidesPerViewParam : 3.3;
    ...
    return (
        <section className="w-full h-full relative">
            ...
            <div className="absolute left-4 right-4 sm:left-16 sm:right-16 !bottom-[-40px] flex flex-row items-center justify-center gap-14">
                <button
                    onClick={() => swiperRef.current?.slidePrev()}
                    disabled={isBeginning}
                    className={`text-primary-granite text-sm font-semibold transition-opacity flex items-center gap-2 ${isBeginning ? 'opacity-40 cursor-not-allowed' : 'opacity-100 hover:opacity-80 cursor-pointer'
                        }`}
